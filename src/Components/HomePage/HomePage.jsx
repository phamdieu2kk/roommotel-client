...
function HomePage() {
    ...
    const pageSize = 6; // số tin mỗi trang

    const startIndex = (currentPage - 1) * pageSize;
    const endIndex = startIndex + pageSize;
    const currentData = dataPost.slice(startIndex, endIndex);

    const cities = [
        {
            name: 'Hồ Chí Minh',
            image: 'https://nld.mediacdn.vn/2017/photo-7-1510398776927.jpg',
        },
        {
            name: 'Hà Nội',
        ...
        {
            name: 'Vũng Tàu',
            image: 'https://etrip4utravel.s3-ap-southeast-1.amazonaws.com/images/tinymce/2021/07/686f3690-e2cb-4502-ac11-bf4944b65500.jpg',
        },
        {
            name: 'Phú Quốc',
            image: 'https://encrypted-tbn0.gstatic.com/licensed-image?q=tbn:ANd9GcQpmy9w1MouaaE-UO5JnPtkxSZ-GDU0pZIiz3RPbiN7CCtvUA9wQmU4JqW_kNoSPkCySOEX6C5sXCAxjkYw1g9h75nAbYuntJZ9II7HtQ',
        },
    ];
        ...
        <div className={cx('wrapper')}>
            <div className={cx('inner')}>
                <div className={cx('header')}>
                    <h1 className={cx('title')}>Nền tảng tìm phòng trọ uy tín hàng đầu</h1>
                    <p className={cx('description')}>
                        Cập nhật nhanh chóng thông tin nhà trọ, căn hộ và dịch vụ hỗ trợ thuê nhà.
                    </p>
                    <p className={cx('description-1')}>Hiện có {dataPost.length} tin đang cho thuê</p>
                </div>
                ...
                <div style={{ textAlign: 'center', marginTop: 20 }}>
                    <Pagination
                        ...
                        showSizeChanger={false} // ẩn lựa chọn số item/page
                    />
                </div>
            </div>
            <div className={cx('filter')}>
                <div className={cx('city-card')}>
                    <img src={cities[cityIndex].image} alt={cities[cityIndex].name} className={cx('city-image')} />
                    <div className={cx('city-actions')}>
                        <button onClick={handlePrev}>◀</button>
                        <button onClick={handleNext}>▶</button>
                    </div>
                    <p className={cx('city-title')}>{cities[cityIndex].name}</p>
                </div>
